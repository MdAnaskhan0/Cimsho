...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-xl font-extrabold text-slate-800">Sub Categories</h2>
    ...
    <div class="flex items-center gap-3">
        <a href="<?= APP_URL ?>/sub-categories/create" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-white text-sm font-semibold transition-all hover:opacity-90" style="background:linear-gradient(135deg,#6366f1,#8b5cf6)">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <h3 class="font-bold text-slate-700 text-sm">Showing <span id="recordCount"><?= count($subCategories) ?></span> of <span id="totalCount"><?= $total ?></span> records</h3>
    ...
    <div class="overflow-x-auto">
        <table class="w-full" id="subCategoriesTable">
            <thead>
            ...
            <tbody class="divide-y divide-slate-50">
                    ...
                    <tr id="emptyRow">
                        <td colspan="8" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <div>
                                    <p class="text-slate-500 font-semibold text-sm">No sub-categories found</p>
                                    <p class="text-slate-400 text-xs mt-1">Create your first sub-category to get started.</p>
                                    <a href="<?= APP_URL ?>/sub-categories/create" class="inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-lg bg-indigo-50 text-indigo-600 text-xs font-semibold hover:bg-indigo-100 transition-colors">
                ...
                <?php else: ?>
                    <?php foreach ($subCategories as $subCategory): ?>
                        <tr class="hover:bg-slate-50/50 transition-colors sub-category-row" data-status="<?= $subCategory['is_active'] ?>" data-name="<?= strtolower(htmlspecialchars($subCategory['name'])) ?>">
                            <td class="px-5 py-3.5">
                                <span class="text-slate-500 text-sm font-mono">#<?= $subCategory['id'] ?></span>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="text-indigo-600 text-xs font-semibold bg-indigo-50 px-2 py-1 rounded"><?= htmlspecialchars($subCategory['category_name'] ?? 'N/A') ?></span>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="text-slate-800 text-sm font-semibold"><?= htmlspecialchars($subCategory['name']) ?></span>
                            </td>
                            <td class="px-5 py-3.5">
                                <code class="text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded"><?= htmlspecialchars($subCategory['slug']) ?></code>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="text-slate-500 text-sm"><?= $subCategory['sort_order'] ?></span>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="badge <?= $subCategory['is_active'] ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500' ?>">
                                    <?= $subCategory['is_active'] ? 'Active' : 'Inactive' ?>
                                </span>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="text-slate-400 text-xs"><?= date('d M, Y', strtotime($subCategory['created_at'])) ?></span>
                            </td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?= APP_URL ?>/sub-categories/edit/<?= $subCategory['id'] ?>" class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center justify-center text-xs transition-colors" title="Edit">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <button onclick="toggleStatus(<?= $subCategory['id'] ?>)" class="w-8 h-8 rounded-lg bg-slate-50 text-slate-600 hover:bg-slate-100 flex items-center justify-center text-xs transition-colors" title="Toggle Status">
                                        <i class="fas <?= $subCategory['is_active'] ? 'fa-eye-slash' : 'fa-eye' ?>"></i>
                                    </button>
                                    <button onclick="confirmDelete(<?= $subCategory['id'] ?>, '<?= htmlspecialchars(addslashes($subCategory['name'])) ?>')" class="w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 flex items-center justify-center text-xs transition-colors" title="Delete">
    ...
    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 transform transition-all">
        <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-lg font-bold text-slate-800">Delete Sub Category</h3>
            </div>
            <p class="text-slate-600 text-sm mb-6">
                Are you sure you want to delete <strong id="deleteSubCategoryName"></strong>? This action cannot be undone.
            </p>
...
<script>
    ...
    function confirmDelete(id, name) {
        ...
        document.getElementById('deleteForm').action = '<?= APP_URL ?>/sub-categories/delete/' + id;
    ...
    function toggleStatus(id) {
        if (confirm('Change sub-category status?')) {
            fetch('<?= APP_URL ?>/sub-categories/toggle-status/' + id, {
                    method: 'POST',
                    headers: {
